...
def build_signal_artifacts(all_metrics, successful_runs, total_signals, avg_metrics=None, error=None):
    """Build compact feedback artifacts for signal-processing candidates."""
    artifacts = {
        "success_rate": f"{successful_runs}/{total_signals} signals completed",
    }
    if error:
        artifacts["error"] = error
        artifacts["suggestion"] = (
            "Ensure run_signal_processing returns a dict containing a non-empty "
            "filtered_signal sequence and terminates within the timeout."
        )
        return artifacts

    if avg_metrics:
        artifacts["objective_breakdown"] = (
            f"composite={avg_metrics['composite_score']:.4f}, "
            f"slope_changes={avg_metrics['slope_changes']:.2f}, "
            f"lag_error={avg_metrics['lag_error']:.4f}, "
            f"avg_error={avg_metrics['avg_error']:.4f}, "
            f"false_reversals={avg_metrics['false_reversals']:.2f}, "
            f"correlation={avg_metrics['correlation']:.4f}, "
            f"noise_reduction={avg_metrics['noise_reduction']:.4f}"
        )

        bottlenecks = []
        if avg_metrics["slope_changes"] > 25:
            bottlenecks.append("too many slope changes; increase smoothing or hysteresis")
        if avg_metrics["lag_error"] > 0.35:
            bottlenecks.append("high recent lag; improve responsiveness to current samples")
        if avg_metrics["avg_error"] > 0.35:
            bottlenecks.append("high average tracking error; reduce bias/drift")
        if avg_metrics["false_reversals"] > 8:
            bottlenecks.append("too many false trend reversals; add trend confirmation")
        if avg_metrics["correlation"] < 0.5:
            bottlenecks.append("low clean-signal correlation; preserve signal structure")
        if avg_metrics["noise_reduction"] < 0.2:
            bottlenecks.append("weak noise reduction; smooth noisy components more")

        artifacts["actionable_hints"] = (
            "\n".join(f"- {hint}" for hint in bottlenecks)
            if bottlenecks
            else "Metrics are balanced; try small parameter refinements."
        )
    ...
    if all_metrics:
        ...
        worst = ranked[0]
        best = ranked[-1]
        artifacts["worst_signal"] = (
            f"composite={worst['composite_score']:.4f}, "
            f"slope_changes={worst['slope_changes']:.2f}, "
            f"lag_error={worst['lag_error']:.4f}, "
            f"false_reversals={worst['false_reversals']:.2f}"
        )
        artifacts["best_signal"] = (
            f"composite={best['composite_score']:.4f}, "
            f"correlation={best['correlation']:.4f}, "
            f"noise_reduction={best['noise_reduction']:.4f}"
        )
    ...
    try:
        ...
        if not hasattr(program, "run_signal_processing"):
            return EvaluationResult(
                ...
                artifacts=build_signal_artifacts([], 0, 0, error="Missing run_signal_processing function"),
            )
        ...
        if successful_runs == 0:
            return EvaluationResult(
                ...
                artifacts=build_signal_artifacts(
                    all_metrics, successful_runs, len(test_signals), error="All test signals failed"
                ),
            )
        ...
        return EvaluationResult(
            metrics=metrics,
            artifacts=build_signal_artifacts(
                all_metrics, successful_runs, len(test_signals), avg_metrics=metrics
            ),
        )
        ...
        return EvaluationResult(
            metrics={"composite_score": 0.0, "overall_score": 0.0, "error": str(e)},
            artifacts=build_signal_artifacts([], 0, 0, error=str(e)),
        )
